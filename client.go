...
// Client代表一个 RPC Client，
// 可能有多个未完成的 Call与单个客户端相关联，
// 并且一个 Client可能同时被多个 goroutine使用
type Client struct {
	cc       codec.Codec //cc 是消息的编解码器，和服务端类似，用来序列化将要发送出去的请求，以及反序列化接收到的响应。
	opt      *Option     //sending 是一个互斥锁，和服务端类似，为了保证请求的有序发送，即防止出现多个请求报文混淆
	sending  sync.Mutex
	...
	closing  bool             // closing 是用户主动关闭的，即调用 Close 方
